...
public:
    ...
    Matrix(Matrix &other) { // перегрузка оператора копирования
        rows = other.rows;
    ...
    bool operator!=(const Matrix &other) const {  // перезгука !=
        if (rows != other.rows || columns != other.columns) {
            return true;
        } else {
            for (size_t i = 0; i < rows; ++i) {
    ...
    bool operator!=(const T &data) const {  // перезгука != для сравнения со скаляром
        if (rows != columns) {
            return true;
        } else {
            for (size_t i = 0; i < rows; ++i) {
    ...
    Matrix mat_alg_add() {
        if (this->rows != this->columns) {
            std::cerr << "Try to get matrix of algebraic addictions from non-square matrix" << std::endl;
            return Matrix(1, 1);
        }
